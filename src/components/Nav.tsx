  ...
  if (isAuthenticated){
    ...
    return (
      <nav className="flex w-full h-24 text-2xl px-5 justify-between items-center" style={{ backgroundColor: '#2485A9', color: '#ffffff' }}>
        ...
        <h1 className="text-5xl">BeKindly</h1>
        <div className="flex items-center">
            <button onClick={() => login()} type="button" className="btn text-3xl text-white pr-4 pt-2 bg-transparent border-hidden hover:bg-transparent hover:opacity-75">Sign In</button>
            <button onClick={() => register()} type="button" className="btn text-3xl text-white bg-transparent border-hidden hover:bg-transparent hover:opacity-75">Sign Up</button>
        </div>
      </nav>
    )
  }
